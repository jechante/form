...
import com.schinta.repository.JobRepository;
...
public class JobResource {
     ...
     * GET  /jobs/:id : get the "id" job.
     *
     * @param title the id of the job to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the job, or with status 404 (Not Found)
     */
    @GetMapping("/jobs-title/{title}")
    @Timed
    public ResponseEntity<Job> getJobByTitle(@PathVariable String title) {
        log.debug("REST request to get Job : {}", title);
        Optional<Job> job = jobRepository.findOneByJobTitle(title);
        return ResponseUtil.wrapOrNotFound(job);
    }
}
