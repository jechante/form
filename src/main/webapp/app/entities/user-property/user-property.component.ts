...
export class UserPropertyComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.userPropertyService
            ...
            .subscribe(
                (res: HttpResponse<IUserProperty[]>) => this.paginateUserProperties(res.body, res.headers),
    ...
    trackId(index: number, item: IUserProperty) {
        return item.id;
    }

    registerChangeInUserProperties() {
        this.eventSubscriber = this.eventManager.subscribe('userPropertyListModification', response => this.loadAll());
    }
    ...
    private paginateUserProperties(data: IUserProperty[], headers: HttpHeaders) {
